    ...
    for (std::map<int, PVRDvrTimer>::iterator tmr = m_timers.begin(); tmr != m_timers.end(); ++tmr) {
        if (tmr->second.strChannelName == currChannel.strChannelName) {
            if ((tmr->second.Timer.startTime < startTime && startTime < tmr->second.Timer.endTime) ||
                ...
                if ((tmr->second.Timer.state == PVR_TIMER_STATE_SCHEDULED ||
                     (tmr->second.Timer.state == PVR_TIMER_STATE_RECORDING)) &&
                    (tmr->second.Status == PVR_STREAM_IS_RECORDING ||
                     tmr->second.Status == PVR_STREAM_START_RECORDING)) {
...
PVR_ERROR PVRDvrData::UpdateTimer(const PVR_TIMER &timer) {
    ...
    if (GetTimer(timer, currTimer)) {
        if (currTimer.Timer.state == PVR_TIMER_STATE_RECORDING &&
            (currTimer.Status == PVR_STREAM_IS_RECORDING || currTimer.Status == PVR_STREAM_START_RECORDING ||
    ...
    if (GetRecording(recording, currRecording)) {
        ...
        if (XBMC->FileExists(recFile.c_str(), false)) {
            ...
            if (UpdateRecording(currRecording)) {
                ...
                XBMC->Log(LOG_NOTICE, "Playback srobbled successfully %s to %i secs", currRecording.Recording.strTitle,
                          lastplayedposition);
                s_triggerRecordingUpdate = true;
                return PVR_ERROR_NO_ERROR;
            }
    ...
    if (GetTimer(myTimer.Timer, currTimer)) {
        ...
        switch (currTimer.Status) {
            case PVR_STREAM_NO_STREAM : {
                if (myTimer.Status != PVR_STREAM_START_RECORDING && myTimer.Status != PVR_STREAM_NO_STREAM)
                    return false;
                break;
            }
            case PVR_STREAM_START_RECORDING : {
                if (myTimer.Status != PVR_STREAM_IS_RECORDING && myTimer.Status != PVR_STREAM_START_RECORDING)
                    return false;
                break;
            }
            case PVR_STREAM_IS_RECORDING : {
                if (myTimer.Status != PVR_STREAM_IS_STOPPING && myTimer.Status != PVR_STREAM_STOPPED &&
                    myTimer.Status != PVR_STREAM_IS_RECORDING)
                    return false;
                break;
            }
            case PVR_STREAM_IS_STOPPING : {
                if (myTimer.Status != PVR_STREAM_STOPPED && myTimer.Status != PVR_STREAM_IS_STOPPING) return false;
                break;
            }
            case PVR_STREAM_STOPPED : {
                if (myTimer.Status != PVR_STREAM_STOPPED) return false;
                break;
            }
        }
...
std::string PVRDvrData::GetTimerString(const PVRDvrTimer &myTimer) {
    ...
    std::string Line =
            "\"" + ChannelName;                                                                              //0
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iClientIndex);                                                         //1
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iParentClientIndex);                                                   //2
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iClientChannelUid);                                                    //3
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.startTime);                                                            //4
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.endTime);                                                              //5
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.state + (myTimer.Timer.state == PVR_TIMER_STATE_RECORDING ? 100 : 0)); //6
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iTimerType);                                                           //7
    Line = Line + "\"|\"" +
           strTitle;                                                                                     //8
    Line = Line + "\"|\"" +
           strEpgSearchString;                                                                           //9
    ...
    Line = Line + "\"|\"" +
           strSummary;                                                                                   //12
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iPriority);                                                            //13
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iLifetime);                                                            //14
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iRecordingGroup);                                                      //15
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.firstDay);                                                             //16
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iWeekdays);                                                            //17
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iPreventDuplicateEpisodes);                                            //18
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iEpgUid);                                                              //19
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iMarginStart);                                                         //20
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iMarginEnd);                                                           //21
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iGenreType);                                                           //22
    Line = Line + "\"|\"" +
           inttostr(myTimer.Timer.iGenreSubType);                                                        //23
    Line = Line + "\"|\"" +
           Plot;                                                                                         //24
    Line = Line + "\"|\"" +
           PlotOutline;                                                                                  //25
...
bool PVRDvrData::ParseTimerString(std::string buffStr, PVRDvrTimer &myTimer) {
    ...
    myTimer.Timer.iClientIndex = strtoint(
            lineVect[1]);                                                               //1
    myTimer.Timer.iParentClientIndex = strtoint(
            lineVect[2]);                                                               //2
    myTimer.Timer.iClientChannelUid = strtoint(
            lineVect[3]);                                                               //3
